...
public class DefaultRestErrorResolver implements RestErrorResolver, MessageSourceAware, InitializingBean {
	...
	private Response createResp(HttpServletRequest request,Exception ex) {
		try {
			if(ex instanceof BizException){
				BizException se=(BizException)ex;
				//异常处理：由于报错->Stream closed 暂不接收request数据，只做异常码和参数处理
				//String tt = StringUtils.collectionToCommaDelimitedString(IOUtils.readLines(request.getInputStream()));
				//log.error("request:"+tt);
				Request req =null;// JsonMapper.nonEmptyMapper().fromJson(tt, Request.class);
				if (req == null) {
					req = new Request();
					req.setCtx(new Context());
				}
				Response res = new Response(req);
				res.setStatus(new Status(se.getCode(),se.getParams()));
				log.error("response:"+JsonMapper.nonAlwaysMapper().toJson(res));
				return res;
			}  
		} catch (Exception e) {
			log.error("createResp EXCEPTION: ",e);
			return null;
		}
		return null;
	}
	...
	@Override
	public Response resolveError(HttpServletRequest request, Object handler, Exception ex) {
		log.error("异常拦截",ex);
		return createResp(request, ex);
	}
